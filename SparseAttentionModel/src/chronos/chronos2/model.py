...
class Chronos2Output(ModelOutput):
    ...
    enc_attention_mask: torch.Tensor | None = None  # [B, S] 0/1 mask after patching (+REG+future)
...
class Chronos2Model(PreTrainedModel):
    ...
    def encode(
        ...
        return encoder_outputs, loc_scale, patched_future_covariates_mask, num_context_patches, attention_mask
    ...
    def forward(
        ...
        **kwargs,   # <-- ADD THIS
        ...
        # ---- HF/Trainer compatibility mapping ----
        if context is None and "input_ids" in kwargs:
            context = kwargs.pop("input_ids")
    
        if context_mask is None and "attention_mask" in kwargs:
            context_mask = kwargs.pop("attention_mask")
    
        if future_target is None and "labels" in kwargs:
            future_target = kwargs.pop("labels")
        ...
        """
        batch_size = context.shape[0]
        encoder_outputs, loc_scale, patched_future_covariates_mask, num_context_patches, enc_attention_mask = self.encode(
            context=context,
            context_mask=context_mask,
            group_ids=group_ids,
            future_covariates=future_covariates,
