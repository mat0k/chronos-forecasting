...
class Chronos2Pipeline(BaseChronosPipeline):
    ...
    def fit(
        ...
        **extra_trainer_kwargs
        ...
        if finetune_mode == "lora":
            ...
            print(f"[LoRA] trainable/total: {trainable} / {total}")
        ...
        backend = getattr(config, "time_attention_backend", "torch")
        attn_type = getattr(config, "time_attention_type", "full")
        require_full_context = (backend == "flash" and attn_type == "windowed_future_global")
        ...
        print("require_full_context(train) =", getattr(train_dataset, "require_full_context", None))
    ...
    def predict(
        ...
        **kwargs,
        ...
        attn_type = getattr(self.model.config, "time_attention_type", "full")
        require_full_context = (backend == "flash" and attn_type == "windowed_future_global")
    ...
    def _predict_fev_window(
        ...
        **predict_kwargs,
        ...
        # Ensure accurate CUDA timing (otherwise timings are mostly kernel launch time)
        is_cuda = torch.cuda.is_available() and next(self.model.parameters()).is_cuda
        if is_cuda:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        if find_adapter_config_file(pretrained_model_name_or_path) is not None:
            ...
            # Extract common HF/Chronos kwargs safely
            device_map = kwargs.get("device_map", None)
            dtype = kwargs.get("dtype", None)
            # Some callers still pass torch_dtype; keep backward compatibility
            if dtype is None and "torch_dtype" in kwargs:
                dtype = kwargs.get("torch_dtype")
            
            time_attention_type = kwargs.get("time_attention_type", None)
            time_attention_backend = kwargs.get("time_attention_backend", None)
            time_local_radius = kwargs.get("time_local_radius", None)
            time_attention_chunk_size = kwargs.get("time_attention_chunk_size", None)
            time_use_landmarks = kwargs.get("time_use_landmarks", None)
            
            # 1) Read adapter config to discover base model
            peft_cfg = PeftConfig.from_pretrained(pretrained_model_name_or_path)
            base_model_id = peft_cfg.base_model_name_or_path
            
            # 2) Load base Chronos2 model (IMPORTANT: Chronos2Model, not Transformers AutoModel)
            base_model = Chronos2Model.from_pretrained(
                base_model_id,
                device_map=device_map,
                dtype=dtype,
                time_attention_type=time_attention_type,
                time_attention_backend=time_attention_backend,
                time_local_radius=time_local_radius,
                time_attention_chunk_size=time_attention_chunk_size,
                time_use_landmarks=time_use_landmarks,
            )
